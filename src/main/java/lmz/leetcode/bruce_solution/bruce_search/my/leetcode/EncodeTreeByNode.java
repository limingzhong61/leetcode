package lmz.leetcode.bruce_solution.bruce_search.my.leetcode;

import lmz.leetcode.graph_parse_ds.search_and_recur.Node;
...
public  class EncodeTreeByNode {
        ...
        while (!queue.isEmpty()) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (root.left != null) {
                    ...
                    queue.add(new Node(nullValue));//补充结点
                }
                if (root.right != null) {
    ...
    public static Node deserialize(String data) {
        ...
        nodes[0] = new Node(Integer.parseInt(split[0]));
        ...
        for (int i = 1; i < split.length; ) {
            ...
            for (int j = start; j < nextStart; j++) {
                ...
                if (!"null".equals(split[i])) {
                    nodes[i] = new Node(Integer.parseInt(split[i]));
                }
