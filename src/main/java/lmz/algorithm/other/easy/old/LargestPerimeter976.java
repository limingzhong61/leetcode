...
public class LargestPerimeter976 {
    /**
     * 排序+贪心
     * leetcode:先对整个数组排序，倒序枚举第 i 个数作为最长边，那么我们只要看其前两个数 test.A[i−2]
     * 和 test.A[i−1]，判断 test.A[i−2]+test.A[i−1] 是否大于 test.A[i] 即可，如果能组成三角形我们就找到了最大周长的三角形，返回答案A[i−2]+test.A[i−1]+test.A[i] 即可。
     * **为什么只需要判断数组中相邻的三个数？**
     * 在固定最后一个数 test.A[i] 时，前两个数需不需要再往前找呢？
     * 如果 test.A[i-2] + test.A[i-1] <= test.A[i] ，这三个数一定不能构成三角形，而A[i-3]以及更往前的数，都小于等于A[i-2]，所以再往前取任何两个数只会让相加的值更小，就更不能满足 test.A[j] + test.A[k] > test.A[i]了 (j<i-2, k<i-1, j<k)。所以如果相邻的数构不成三角形，就不需要再固定第三个数并往前找两个数了。
     * 如果 test.A[i-2] + test.A[i-1] > test.A[i]j，这三个数可以构成三角形，再往前找只会让周长变短，所以也不用再往前了。
     * 综上，只需要判断相邻的三个数。
     */
